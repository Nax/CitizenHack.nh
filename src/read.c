...
static void seffect_upgrade(struct obj **);
...
recharge(struct obj* obj, int curse_bless)
{
    ...
    if (obj->oclass == WAND_CLASS) {
        ...
        if (obj->otyp == WAN_WISHING
                      || (n > 1 && (n * n * n > rn2(7 * 7 * 7)))) { /* recharge_limit */
...
static void
seffect_upgrade(struct obj **sobjp)
{
    struct obj *sobj = *sobjp;
    int otyp = sobj->otyp;
    boolean sblessed = sobj->blessed;
    ...
    for (int i = 0; upgrade_list[i].otyp_source; ++i) {
        ...
        if (obj->otyp == src) {
            if (scursed || sblessed || !(!objects[src].oc_magic && objects[dst].oc_magic))
    ...
    if (!candidate_count) {
        Your("%s warps %s for a moment.", xname(obj), scursed ? "maliciously" : "strangely");
    }

    Your("%s warps %s and turn into something else!", xname(obj), scursed ? "maliciously" : "strangely");

    if (!already_known)
        (void) learnscrolltyp(SCR_UPGRADE);
